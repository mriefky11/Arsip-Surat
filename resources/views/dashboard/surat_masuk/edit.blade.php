...
@section('container')
    <div class="container-fluid">
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800 fw-bold">Edit Surat Masuk</h1>

        {{-- form control --}}
        <div class="card shadow mb-4 py-5 px-3">
            <form action="/dashboard/surat_masuk/{{ $suratMasuk->id }}" method="POST" class="px-4"
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select form-select-sm" name="category" id="category">
                        @foreach ($category as $item)
                            @if (old('category', $suratMasuk->category_id) == $item->id)
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control form-control-sm @error('penerima_ids') is-invalid @enderror"
                        ...
                        value="{{ old('penerima_ids', $suratMasuk->penerima_ids) }}">
                    @error('penerima_ids')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control form-control-sm @error('tembusan') is-invalid @enderror"
                        ...
                        value="{{ old('tembusan', $suratMasuk->tembusan) }}">
                    @error('tembusan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input multiple type="file" class="form-control @error('file_pdf') is-invalid @enderror"
                        id="file_pdf" name="file_pdf" autofocus value="{{ old('file_pdf', $suratMasuk->file_pdf) }}">
                    @error('file_pdf')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-5">
                    ...
                    <input id="x" type="hidden" name="perihal"
                        value="{{ old('perihal', $suratMasuk->perihal) }}"required autofocus>
                    @error('perihal')
                        <p>{{ $message }}</p>
                    @enderror
